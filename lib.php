...
function aidialogue_save_criteria(int $aidialogueid, stdClass $data): void {
    ...
    if ($todelete) {
        ...
        // Null out criterionid on turns so transcript content is preserved but the
        // broken FK reference is removed. The turn still appears in get_turns() output.
        $DB->execute(
            "UPDATE {aidialogue_turn} SET criterionid = NULL WHERE criterionid $delsql",
            $delparams,
        );
